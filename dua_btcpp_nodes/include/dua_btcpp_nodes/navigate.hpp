...
#include <dua_movement_interfaces/action/navigate.hpp>
...
/**
 * Node that navigates to a target position.
 */
class DUA_BTCPP_NODES_PUBLIC NavigateNode : public BT::StatefulActionNode
{
public:
  /**
   * @brief Constructor.
...
private:
  ...
  simple_actionclient::Client<Navigate>::SharedPtr action_client_ = nullptr;

  /* Pointer to current goal. */
  rclcpp_action::ClientGoalHandle<Navigate>::SharedPtr current_goal_ = nullptr;

  /* Current result future. */
  std::unique_ptr<std::shared_future<rclcpp_action::ClientGoalHandle<Navigate>::WrappedResult>> current_res_future_ = nullptr;
