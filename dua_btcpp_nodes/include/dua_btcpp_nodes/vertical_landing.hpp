...
/**
 * Node that executes a vertical landing.
 */
class DUA_BTCPP_NODES_PUBLIC VerticalLandingNode : public BT::StatefulActionNode
{
public:
  /**
   * @brief Constructor.
...
private:
  ...
  simple_actionclient::Client<Landing>::SharedPtr action_client_ = nullptr;

  /* Pointer to current goal. */
  rclcpp_action::ClientGoalHandle<Landing>::SharedPtr current_goal_ = nullptr;

  /* Current result future. */
  std::unique_ptr<std::shared_future<rclcpp_action::ClientGoalHandle<Landing>::WrappedResult>> current_res_future_ = nullptr;
