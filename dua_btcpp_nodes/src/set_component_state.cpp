  ...
  spin_(spin)
{
  ...
  if (!isBlackboardPointer(bb_service_name)) {
    ...
    service_client_ = clients_cache_->get_service_client<SetBool>(bb_service_name, wait_server);
  }
...
BT::NodeStatus SetComponentState::tick()
{
  ...
  if (service_client_ == nullptr) {
    ...
    service_client_ = clients_cache_->get_service_client<SetBool>(service_name.value(), wait_server_);
  }

  // Try to set the state of the component
