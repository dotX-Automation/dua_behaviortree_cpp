...
BT::NodeStatus NavigateNode::onStart()
{
  ...
  current_goal_ = action_client_->send_goal_sync(nav_goal, spin_, goal_timeout_ms);
...
BT::NodeStatus NavigateNode::onRunning()
{
  ...
  if (current_res_future_ == nullptr) {
    current_res_future_ =
      std::make_unique<std::shared_future<rclcpp_action::ClientGoalHandle<Navigate>::WrappedResult>>(
...
void NavigateNode::onHalted()
{
  ...
  if (current_goal_ != nullptr) {
    ...
    try {
      ...
      RCLCPP_FATAL(
        ros2_node_->get_logger(),
        "Failed to cancel Navigate on halt: %s",
        e.what());
    }
    current_goal_.reset();
  }
